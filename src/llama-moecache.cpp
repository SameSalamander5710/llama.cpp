...
void llama_moe_cache_init(const llama_model & model, int32_t n_slots, int32_t max_inserts, bool cpu_only) {
    ...
    [&]() {
        ...
        for (size_t il = 0; il < model.layers.size(); ++il) {
            ...
            if (cpu_only) {
                // reuse ggml_backend_buffer_is_pinned from ffn-prefetch: skip buffers pinned by accelerator devices (e.g. Vulkan_Host)
                ggml_backend_buffer_t buf = l.ffn_up_exps->buffer;
                ggml_backend_buffer_type_t router_buft = l.ffn_gate_inp->buffer ? ggml_backend_buffer_get_type(l.ffn_gate_inp->buffer) : nullptr;
                ggml_backend_dev_t router_dev = router_buft ? ggml_backend_buft_get_device(router_buft) : nullptr;
                if (router_dev && ggml_backend_buffer_is_pinned(buf, router_dev)) {
                    continue; // exps are in a pinned host buft owned by the device backend: skip
                }
