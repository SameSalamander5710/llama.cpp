...
void llama_context::sched_reserve() {
    ...
    sched.reset(ggml_backend_sched_new(backend_ptrs.data(), backend_buft.data(), backend_ptrs.size(), max_nodes, cparams.pipeline_parallel, cparams.op_offload, cparams.prefetch_weights));
    ...
    // reserve pp (prompt processing) graph first so that buffers are only allocated once
    {
        ...
        if (!gf) {
            if (cparams.pipeline_parallel) {
                ...
                cparams.pipeline_parallel = false;
                sched.reset(ggml_backend_sched_new(backend_ptrs.data(), backend_buft.data(), backend_ptrs.size(), max_nodes, false, cparams.op_offload, cparams.prefetch_weights));
